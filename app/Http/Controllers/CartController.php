...
class CartController extends Controller
{
    ...
    public function create_cart()
    {
        ...
        if ($gs->cart_amount != 0 || $gs->cart_amount != '')
        {
            
            if ($gs->cart_amount * $conversion_rate <= $total * $conversion_rate)
            {
               
                DB::table('carts')->where('user_id', '=', Auth::user()
                    ->id)
                    ->update(array(
                    'shipping' => 0
                ));
    ...
    public function add_item(request $request, $id, $variantid, $varprice, $varofferprice, $qty)
    {
        ...
        if (!empty($user))
        {
            $cart_of_user = Cart::where('variant_id', $variantid)->where('user_id', $user->id)
                ->get();
            $cart_table = Cart::where('variant_id', $variantid)->where('user_id', $user->id)
                ->first();

            if (count($cart_of_user) > 0)
            {
                ...
                if (isset($cart_table))
                {
                    ...
                    if ($cart_table->variant_id == $variantid)
                    {
                        ...
                        {
                            notify()
                                ->error('Variant is already available in cart !');
                            return back();
                        }
	...
	/**
     * To recalculate the product price
     * 
     */
	...
	/**
     * To recalculate the product price with coupoun code on product category
     * 
     */
	...
	/**
     * To recalculate the cart products
     * 
     */
	...
	/**
     * To update cart products
	 * @param  Illuminate\Http\Request  $request, Product ID $id
     * @return Back()
     * 
     */
	...
	/*
	* empty the full cart with coupon code
	* 
	**/
