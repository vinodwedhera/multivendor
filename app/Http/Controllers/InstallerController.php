...
class InstallerController extends Controller
{
    ...
    public function step2(Request $request)
    {
        ...
        try{
            ...
            if($errorcode == 2002){
                notify()->error('2002 ! Invalid Database Host !');
                return redirect()->route('get.step2')->withInput($request->except('DB_PASSWORD'));
            }

             notify()->error('Oops ! Something went wrong Please try againvbgb.');
             return redirect()->route('get.step2')->withInput($request->except('DB_PASSWORD'));
    ...
    public function getstep3()
    {
        ...
        catch(\Exception $e)
        {
            ...
            {
                notify()->error('Error Connecting in Database Please check details !');
                return redirect()->route('get.step2');
            }

            if($errorcode == 2002){
                notify()->error('Invalid Database host !');
                return redirect()->route('get.step2');
            }
    ...
    public function storeStep3(Request $request)
    {
        ...
        {   
            $Logo = @file_get_contents('../public/images/genral/' . $newGenral->logo);
        ...
        if ($file = $request->file('favicon'))
        {
            ...
            $favicon = @file_get_contents('../public/images/genral/' . $newGenral->fevicon);
            if ($favicon)
            {
    ...
    public function storeStep4(Request $request)
    {
        ...
        $request->validate(['name' => 'required|string|max:255', 'email' => 'required|string|email|max:255|unique:users', 'password' => 'required|string|min:6|confirmed', 'password_confirmation' => 'required', 'profile_photo' => 'mimes:jpg,jpeg,png,bmp', 'country' => 'required', 'state_id' => 'required', 'city_id' => 'required']);
