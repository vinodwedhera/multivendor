...
class VenderOrderController extends Controller
{
    ...
    {	
    	...
    	return $order = Order::where('order_id',$id)->where('status','=','1')->first();
                  ...
                  if($inv->seller->role_id == 'v'){
                    ...
                    if( isset( $response_object->result ) ){
                       if($response_object->result == 'error'){
                          ...
                          $conv_id = "{$from}_{$defCurrency->currency->code}";
                          ...
                          $convr_curr = $defCurrency->currency->code;
                          $defcurrate = round(($response_object->rates->$convr_curr),4);
                       }
                   }else{
                          $conv_id = "{$from}_{$defCurrency->currency->code}";
                          $string = file_get_contents("http://free.currconv.com/api/v7/convert?q=$conv_id&compact=ultra&apiKey=".$api_key);
                    ...
                    $actualprice    =  round(($inv->price*$inv->qty)*$defcurrate,2);
                    $actualtax      =  round(($inv->tax_amount*$inv->qty)*$defcurrate,2);
                    $actualshipping =  round($inv->shipping*$defcurrate,2);
